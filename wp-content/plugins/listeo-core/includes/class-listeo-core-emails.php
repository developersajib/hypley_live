  ...
  //       )
	...
	function new_conversation_mail($args){
		...
		$args = array(
			...
			'conversation_url' => get_permalink('listeo_messages_page').'?action=view&conv_id='.$conversation_id.'&user='.$userhash,
			'sender'		=> $sender,
			);
    ...
    */
	...
	public static function send( $emailto, $subject, $body , $activation_link='', $reply_to=''){
		...
		if($reply_to != ''){
			$headers .='Reply-To: '.$reply_to.' <hypleyteam@hypley.com>';
		}
