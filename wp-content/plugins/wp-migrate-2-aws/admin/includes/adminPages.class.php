...
class WPM2AWS_AdminPages
{
    ...
    public static function loadIAMform()
    {
        ...
        if (false === get_option('wpm2aws_valid_licence') || '' === get_option('wpm2aws_valid_licence') || empty(get_option('wpm2aws_valid_licence'))) {
            ...
            if (defined('WPM2AWS_TESTING')) {
                echo "Bucket: " . get_option('wpm2aws-aws-s3-bucket-name');

                echo "<div>";
                echo "<div class='button button-default' id='wpm2aws-get-dynamic-progress'>Check Progress</div>";
                echo "<br>";
                echo "<div><p id='wpm2aws-dynamic-output'></p></div>";
                echo "</div>";

                self::makeDevTestingButtons();
            }
    ...
    private static function generateInputSection($title, $formAction, $formElements, $reference, $label, $additionalInfo = null)
    {
        ...
        <div class="wpm2aws-inputs-item">
            ...
            if (!empty($formElements)) : ?>
                ...
                <p><input type="submit" class="button" value="<?php echo esc_attr(__($label, 'migrate-2-aws')); ?>" /></p>
                </form>
            <?php else : ?>
        ...
        <div class="wrap">
            ...
            <div style="display:inline-block;">
                <div style="margin-left:5px;margin-top:10px;">
                    ...
                    <a href="https://seahorse-data.com" target="_blank">
    ...
    private static function makeDevTestingButtons()
    {
        ?>

        <h3>DEV TESTING</h3>
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>?action=wp_wpm2aws-dev-testing">
        <input type="hidden" name="action" value="wpm2aws-dev-testing" />
            <?php wp_nonce_field('wpm2aws-dev-testing'); ?>
            <p>
            <input type="submit" class="button button-primary" name="wpm2aws-test-iam-credentials" value="Test IAM Credentials" />
            <input type="submit" class="button button-primary" name="wpm2aws-test-get-region" value="Test Get Default Region" />
            <input type="submit" class="button button-primary" name="wpm2aws-test-set-region" value="Test Set Region" />
            <input type="submit" class="button button-primary" name="wpm2aws-test-set-region" value="Test Set Region" />
            <input type="submit" class="button button-primary" name="wpm2aws-test-get-existing-buckets" value="Test Get Bucket List" />
            <input type="submit" class="button button-primary" name="wpm2aws-test-create-s3-bucket" value="Test Create S3 Bucket" />
            <input type="submit" class="button button-primary" name="wpm2aws-test-delete-s3-bucket" value="Test Delete S3 Bucket" />
            <input type="submit" class="button button-primary" name="wpm2aws-test-upload-s3-bucket" value="Test Upload To S3 Bucket" />
            <input type="submit" class="button button-primary" name="wpm2aws-test-empty-s3-bucket" value="Test Remove Files From S3 Bucket" />
            <input type="submit" class="button button-primary" name="wpm2aws-test-create-lightsail" value="Test Create Lightsail" />
            <input type="submit" class="button button-primary" name="wpm2aws-test-export-database" value="Test Export Database" />
            <input type="submit" class="button button-primary" name="wpm2aws-test-export-filesystem" value="Test Export Filesystem" />
            <input type="submit" class="button button-primary" name="wpm2aws-test-clear-all-settings" value="Clear All Settings" />
            <input type="submit" class="button button-primary" name="wpm2aws-test-clear-lightsail-display" value="Clear AWS Display" />
            <input type="submit" class="button button-primary" name="wpm2aws-test-get-all-s3-regions" value="Get S3 Regions" />
            <input type="submit" class="button button-primary" name="wpm2aws-test-sh-controlled-bucket-upload" value="SH Bucket Create & Upload" />
            <!-- <input type="submit" class="button" name="wpm2aws-test-xxxxxx" value="Test-xxxxxx" /> -->

            <!-- <input type="hidden" name="wpm2aws-process-once" value="single"/>
            <input type="submit" class="button button-primary" name="wpm2aws-process-once-submit" value="Process Once" />
            <input type="hidden" name="wpm2aws-process-all" value="all"/>
            <input type="submit" class="button button-primary" name="wpm2aws-process-all-submit" value="Process All" /> -->

            <input type="submit" class="button button-primary" name="wpm2aws-export-db" value="Export DataBase" />

            <input type="submit" class="button button-primary" name="wpm2aws-test-admin-email" value="Email Admin" />



            <?php  // wp_nonce_field('wpm2aws-process');?>

            <!-- <a href="<?php // echo wp_nonce_url( admin_url( 'admin.php?page=wpm2aws&process=wpm2aws-single'), 'wpm2aws-process' );?>">Old Single</a> -->
            </p>
        </form>
            ...
            if (defined('WPM2AWS_LIMIT_ZIP_DIR_SIZE') && true === WPM2AWS_LIMIT_ZIP_DIR_SIZE) {
                ...
                if (!empty($scannedFs)) {
                    ...
                    if (!empty($excludedDirs)) {
                        ...
                        $html .= '</ul>';
                        $html .= '<p><em>Please contact <a href="https://www.seahorse-data.com/wp-on-aws-support-portal/">Seahorse Support</a> if required.</em></p>';
                        $html .= '</div>';
                        $html .= '</div>';
                    } else {
    ...
    private static function makeResetButton()
    {
        ...
        $html = '';
        $html .= '<form class="wpm2aws-align-right" method="post" action="' . esc_url(admin_url('admin-post.php')) . '?action=' . $formAction . '">';
        $html .= '<input type="hidden" name="action" value="' . $formAction . '" />';
        $html .=  wp_nonce_field($formAction);
        $html .=  '<input type="submit" class="button button-secondary" name="wpm2aws-reset-all-settings" value="' . $btnLabel . '" />';
        $html .=  '</form>';
    ...
    private static function makeDataBaseDownloadSection($inMainSection = false)
    {
        ...
        if ($inMainSection) {
            ...
            if (false !== $dbSizeWarning && '' !== $dbSizeWarning) {
                ...
                $html .= '<a href="https://www.seahorse-data.com/contact/" target="_blank">Contact Us</a>';
                $html .= '</p> ';
            }
        }
    ...
    private static function makeConfirmedLightsailInfoSection()
    {
        if (get_option('wpm2aws-lightsail-instance-details') !== false) {
            ...
            // 'name' => $name, 'region' => $region, 'publicIp'


            echo '<div class="wpm2aws-inputs-row" style="border-color:#DD6B10;width:97%;padding:15px 10px;display:block;">';
            echo '<h3>AWS Instance Launch Activated Successfully</h3>';
            echo '<h4>Your instance is currently being compiled and will be available shortly.</h4>';
            echo '<h4>An email will be sent to the email address associated with your licence key with further information and active links.</h4>';
            ...
            echo '<li> <strong>AWS Instance IP: </strong>' . $lightsailDetails['publicIp'] . '</li>';
            echo '<li> <strong>Source URL: </strong>' . get_bloginfo('wpurl') . '</li>';
            ///echo '<li> <a href="http://' . $lightsailDetails['publicIp'] . '/wp-admin/admin.php?page=wpm2aws" target="_blank">Site Login</a> <span style="font-size:12px;"><i>(Login with your existing user access details)</i></span></li>';
            echo '<li> ======================================== </li>';
            echo '</ul>';
            ...
            /// show this to the review user
             if (
                ...
                echo '<p>This temporary site will remain active for review for 36 hours after launch.</p>';
                echo '<p>At this point it will be shut-down unless users choose to <a target="_blank" href="https://www.seahorse-data.com/pricing/">purchase a plan or licence</a> for ongoing WordPress Hosting on AWS</p>';
                echo '<p>To migrate a site to your own AWS account <a target="_blank" href="https://www.seahorse-data.com/pricing/">please click here to select a licence</a></p>';

                    // echo '<ul>';
                    // echo '<li>Tier 1: <a target="_blank" href="https://www.seahorse-data.com/checkout?edd_action=add_to_cart&download_id=1330&edd_options[price_id]=1">Add to Cart</a></li>';
                    // echo '<li>Tier 2: <a target="_blank" href="https://www.seahorse-data.com/checkout?edd_action=add_to_cart&download_id=7318&edd_options[price_id]=1">Add to Cart</a></li>';
                    // echo '<li>Tier 3: <a target="_blank" href="https://www.seahorse-data.com/checkout?edd_action=add_to_cart&download_id=7320&edd_options[price_id]=2">Add to Cart</a></li>';
                    // echo '<li>Tier 4: <a target="_blank" href="https://www.seahorse-data.com/checkout?edd_action=add_to_cart&download_id=7322&edd_options[price_id]=2">Add to Cart</a></li>';
                    // echo '<li>Tier 5: <a target="_blank" href="https://www.seahorse-data.com/checkout?edd_action=add_to_cart&download_id=7325&edd_options[price_id]=2">Add to Cart</a></li>';
                    // echo '<li>Tier 6: <a target="_blank" href="https://www.seahorse-data.com/checkout?edd_action=add_to_cart&download_id=7327&edd_options[price_id]=2">Add to Cart</a></li>';
                    // echo '</ul>';
            ...
            if (defined('WPM2AWS_TESTING')) {
                ?>
                <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>?action=wpm2aws-dev-testing">
                <input type="hidden" name="action" value="wpm2aws-dev-testing" />
                <?php wp_nonce_field('wpm2aws-dev-testing'); ?>
                <p>
                <input type="submit" class="button button-primary" name="wpm2aws-test-clear-lightsail-display" value="Clear AWS Display" />
