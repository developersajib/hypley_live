...
function wpm2aws_register_licence($licence)
{
    ...
    $response = wp_remote_post(
        'https://wponaws.migration.seahorse-data.com/api/migration/validateLicence',
        // 'http://localhost/wpm2aws-migration-api/public/api/migration/validateLicence',
        array(
...
function wpm2awsUserSetup()
{
    if (
        ...
        wp_die('<a href="https://www.seahorse-data.com/contact/" target="_blank" class="button">Contact Seahorse</a>');
    }
...
function setDbSizeOption()
{
    ...
    try {
        ...
        set_transient('wpm2aws_admin_error_notice_' . get_current_user_id(), __('Error! DB Download Failed: Get Database Size: ' . $e->getMessage(), 'migrate-2-aws'));
        exit(wp_safe_redirect(admin_url('/admin.php?page=wpm2aws')));
    ...
    /* Set User Warning if DB is Large & possibly problematic */
    if (defined('WPM2AWS_MAX_SAFE_DB_SIZE') && round($dbSize) >= (int)WPM2AWS_MAX_SAFE_DB_SIZE) {
        wpm2awsAddUpdateOptions('wpm2aws_db_size_warning', $dbSize);
    } else {
        delete_option('wpm2aws_db_size_warning');
    }
