...
if (!class_exists('WPM2AWS_Settings')) {
    class WPM2AWS_Settings
    {
        ...
        private static function checkAwsforWpPlugin() {
            $isActive = is_plugin_active( 'amazon-polly/amazonpolly.php' );
            if ($isActive === true) {
              $pluginData = get_plugin_data( WP_PLUGIN_DIR."/amazon-polly/amazonpolly.php");
              if($pluginData['Version'] > '4.2.2') {
                self::$settingsErrors[] = 'AWS for WordPress plugin is active on this site and running a version which coflicts with WP on AWS. Please disable this plugin for the duration of the cloning process.';
                }
            }
            return $isActive;
        }
        ...
        public function run()
        {
            ...
            //     $this->runFullMigration();
                ...
                // $this->runDevTest();
            } elseif (isset($_GET['action']) && $_GET['action'] === 'wpm2aws-dev-testing') {
                $this->runDevTest();
            } elseif (isset($_GET['action']) && $_GET['action'] === 'wp_wpm2aws-dev-testing') {
                $this->runDevTest();
            ...
            // } elseif (isset($_GET['action']) && ($_GET['action'] === 'wp_wpm2aws-downloader-once' || $_GET['action'] === 'wp_wpm2aws-downloader-all')) {
            //     $this->runDetachedDownloader();
            ...
            // } elseif (isset($_GET['action']) && ($_GET['action'] === 'wp_wpm2aws-admin-uploader-once' || $_GET['action'] === 'wp_wpm2aws-admin-uploader-all')) {
            //     $this->runDetachedAdminUploader();
                ...
                $this->runFsUploader();
            // } elseif (isset($_GET['action']) && $_GET['action'] === 'wpm2aws-run-fs-download') {
            //     $this->runFsDownloader();
         ...
         * @return void
         */
        private function loadAdminRegionFiles()
        {
            // require_once WPM2AWS_PLUGIN_DIR . '/admin/classes/adminMigrate.class.php';
            // require_once WPM2AWS_PLUGIN_DIR . '/admin/includes/adminFunctions.php';
            // require_once WPM2AWS_PLUGIN_DIR . '/admin/includes/adminValidations.php';
         ...
         * @return void
         */
        private function loadAdminS3BucketFiles()
        {
            // require_once WPM2AWS_PLUGIN_DIR . '/admin/classes/adminMigrate.class.php';
            // require_once WPM2AWS_PLUGIN_DIR . '/admin/includes/adminFunctions.php';
            // require_once WPM2AWS_PLUGIN_DIR . '/admin/includes/adminValidations.php';
         ...
         * @return void
         */
        private function loadAdminLightsailFiles()
        {
            // require_once WPM2AWS_PLUGIN_DIR . '/admin/classes/adminMigrate.class.php';
            // require_once WPM2AWS_PLUGIN_DIR . '/admin/includes/adminFunctions.php';
            // require_once WPM2AWS_PLUGIN_DIR . '/admin/includes/adminValidations.php';
        ...
        private function loadAdminPageFiles()
        {
            ...
            require_once WPM2AWS_PLUGIN_DIR . '/admin/includes/adminMenu.class.php';
            require_once WPM2AWS_PLUGIN_DIR . '/admin/includes/welcomePanel.class.php';
            require_once WPM2AWS_PLUGIN_DIR . '/admin/includes/adminPages.class.php';
            require_once WPM2AWS_PLUGIN_DIR . '/admin/includes/adminPagesConsole.class.php';
            require_once WPM2AWS_PLUGIN_DIR . '/inc/migration-form.php';
            require_once WPM2AWS_PLUGIN_DIR . '/inc/migration-form-template.php';
        }

        /**
         * Loads the Administrator Interface
         *
         * @return void
         */
        public function loadAdminPage()
        {
            $adminPage = new WPM2AWS_AdminPage;
        ...
        public function loadAPIFiles()
        {
            ...
            // require WPM2AWS_PLUGIN_DIR . '/admin/classes/apiGlobal.class.php';
        ...
        public function runFullMigration()
        {
            ...
            if (isset($_POST["wpm2aws-demo-full-migration"])) {
                return $instance = $migrate->createLightsailInstance();
            }
        }
        ...
        public function runFullMigrationZipped()
        {
            ...
            // if (isset($_POST["wpm2aws-demo-full-migration"])) {
            //     return $instance = $migrate->createLightsailInstanceZipped();
            // }
        }
        ...
        public function runDevTest()
        {
            ...
            $this->loadAPIFiles();
            $this->loadUploaderFiles();
            if (isset($_POST["wpm2aws-test-iam-credentials"])) {
                $migrate->verifyIamCredentials();
            } elseif (isset($_POST["wpm2aws-test-get-region"])) {
                $migrate->getAwsRegion();
            } elseif (isset($_POST["wpm2aws-test-set-region"])) {
                $migrate->setAwsRegion();
            } elseif (isset($_POST["wpm2aws-test-get-existing-buckets"])) {
                $migrate->getExistingS3Buckets();
            } elseif (isset($_POST["wpm2aws-test-sh-controlled-bucket-upload"])) {
                $migrate->createS3Bucket(true);
            } elseif (isset($_POST["wpm2aws-test-create-s3-bucket"])) {
                $migrate->createS3Bucket(false);
            } elseif (isset($_POST["wpm2aws-test-upload-s3-bucket"])) {
                $migrate->uploadToS3Bucket();
            } elseif (isset($_POST["wpm2aws-test-empty-s3-bucket"])) {
                $migrate->emptyS3Bucket();
            } elseif (isset($_POST["wpm2aws-test-delete-s3-bucket"])) {
                $migrate->deleteS3Bucket();
            } elseif (isset($_POST["wpm2aws-test-create-lightsail"])) {
                return $instance = $migrate->createLightsailInstance();
            } elseif (isset($_POST["wpm2aws-test-export-database"])) {
                $migrate->exportDatabase();
            } elseif (isset($_POST["wpm2aws-test-export-filesystem"])) {
                $migrate->exportFileSystem();
            } elseif (isset($_POST["wpm2aws-test-clear-all-settings"])) {
                $migrate->clearAllSavedOptions(false);
            } elseif (isset($_POST["wpm2aws-test-clear-lightsail-display"])) {
                $migrate->clearLightsailDisplayOptions();
            } elseif (isset($_POST["wpm2aws-test-get-all-s3-regions"])) {
                $migrate->getS3Regions();
            } elseif (isset($_POST["wpm2aws-demo-full-migration"])) {
                return $instance = $migrate->createLightsailInstance();
            } elseif (isset($_POST["wpm2aws-full-migration-admin"])) {
                return $instance = $migrate->createLightsailInstanceAdmin();
            } elseif (isset($_POST["wpm2aws-export-db"])) {
                $this->loadDbDownloaderFiles();
                return $migrate->exportDb();
            } elseif (isset($_POST["wpm2aws-process-once-submit"]) || isset($_POST["wpm2aws-process-all-submit"])) {
                new WPM2AWS_Uploader();
            } elseif (isset($_POST["wpm2aws-test-admin-email"])) {
                return $email = $this->emailAdmin('Test Email', 'This is a test Email');
                ...
                print_r($_GET);
                echo "<br>";
                wp_die('No Function for this Button');
            }
        }


        public function emailAdmin($subject, $message)
        {
            add_action('admin_init', array($this,'sendAdminEmail'));
        }

        public function sendAdminEmail($sbj = 'Test Subject', $msg = 'Test Contents')
        {
            ...
            echo "Email: " . get_bloginfo('admin_email');
        }
        ...
        private function checkS3BucketAvailable()
        {
            ...
            if ($s3->checkBucketExists() === 'true' || $s3->checkBucketExists() === true) {
                return;
            }

            $s3->abortWithErrorMessage("Error! Upload Destination Does Not Exist. Click 'Reset All' to re-run.");
            return;
        }
    }
}
